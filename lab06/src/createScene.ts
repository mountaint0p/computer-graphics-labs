...
class Playground {
  public static CreateScene(
   ...
   // Scene, Camera and Light setup
    ...
    let znear = 1;
    let zfar = 1000;
    let myProjection = BABYLON.Matrix.PerspectiveFovLH(fov, aspect, znear, zfar);
    ...
    function makeRotationMatrixX(angle){
    ...
    function makeRotationMatrixY(angle){
        const cos = Math.cos(angle);
        const sin = Math.sin(angle);
        var rotationMatrixY = 
        [cos, 0, -sin, 0,   // <- i
    ...
    function makeRotationMatrixZ(angle){
        const cos = Math.cos(angle);
        const sin = Math.sin(angle);
        var rotationMatrixZ = 
        [cos,  sin, 0, 0,   // <- i
    ...
    function makeWorldMatrix(scaling, rotation, translation){
